   ...
   namespace
   {
      ...
      T parse_params(const std::string &body)
      {
         ...
         try
         {
            ...
            { // EOS_RETHROW_EXCEPTIONS does not re-type these so, re-code it
...
#define CHAIN_RO_CALL_WITH_400(call_name, http_response_code) CALL_WITH_400(chain, ro_api, chain_apis::read_only, call_name, http_response_code)
   ...
   void chain_api_plugin::plugin_startup()
   {
      ...
      _http_plugin.add_api({CHAIN_RO_CALL(get_info, 200)}, appbase::priority::medium_high);
      _http_plugin.add_api({CHAIN_RO_CALL(get_activated_protocol_features, 200),
